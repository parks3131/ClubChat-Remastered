...
| **Outbound mail** | A `Mailer` port; **provider deliberately not yet chosen** - [ADR-0019](../decisions/0019-outbound-mail-is-a-port-with-a-deferred-provider.md) | One method, `send`. Development and CI run `LoggingMailer`, which writes the message and its reset URL to the process logger; tests run `RecordingMailer` and assert on it. Choosing a provider is a DNS wait, not a code change, so it was decoupled from the feature that needed it. **The API refuses to boot in production without a real transport** - that guard is what stops the log-only path shipping. |
