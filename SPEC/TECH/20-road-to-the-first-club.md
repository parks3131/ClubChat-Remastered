...
- *A database restore from a real backup has been performed once.* **DONE 2026-08-25.**
  `scripts/drills/restore-drill.mjs` restored Neon history onto a new branch and proved it with
  queries rather than with the API's success code: schema present, real row counts, nothing newer
  than the restore point, referential integrity, and a committed write. It refuses a production
  target by name and never calls the endpoint that rewrites an existing branch. The first real run
  also found a defect in the drill itself, which had counted the four direct-message channels as
  orphans because DM channels legitimately carry no club.
- *A deliberately parked outbox event and a deliberately raised 5xx each reached a human through
  the monitoring path - forced, not assumed.* **The 5xx half is DONE 2026-08-25** and reached the
  founder's phone by itself, issue `021d94c07bb3425e8e0855d42390de21`. Getting there meant fixing
...
| A database restore from a real backup performed once | **Done 2026-08-25.** This row said "Not done ... it has never been run" until 2026-08-27, while the exit-criteria list above it and the branch-state table below it both said done: it was written before the run and never revisited. The run is not in doubt. `scripts/drills/restore-proof.mjs` carries its evidence in a code comment - the first real run reported production's four direct-message channels as orphans, which is a fact about production data that only a real restore could have produced, and excluding `scope = 'dm'` was the fix |
...
| Backup restore, forced alert, symbolicated trace | **Two of three done, 2026-08-25.** The restore drill ran against real Neon and proved the restore with queries rather than with an API success code. A forced 5xx reached the founder's phone through a Sentry rule filtered to `environment: production`, which first required fixing a DSN that had been invented since the deploy and was refusing every event with a 403. **Still open:** the parked outbox event, and symbolicated traces, which need the Sentry auth token for source map upload |
