...
| ~~**Over-the-air updates**~~ | **Done 2026-08-27.** A JavaScript-only fix reaches installed phones through `eas update` with no store release and no Apple review; five shipped that day. A change to a *native* package still needs a build, because the runtime version is a fingerprint of exactly that - which is the protection working, not a gap. See [Deployment](../TECH/21-deployment.md) |
| ~~**An announcement that also mentions somebody buzzes that person twice**~~ | **Done 2026-08-16**, and it was the fix this entry described: the announcement's **push** list subtracts the mentioned, its **row** list does not. One phone, one buzz; two inbox rows, because they say different things and clear against different things. Originally: one phone, one message, two notifications - "Riley: kit order closes Friday" and "Riley mentioned you". It pre-dated per-message push and was made visible by it, since the same rule was already applied correctly one branch over. The existing test asserted the two rows and stopped, so the assertion that was missing had to be added rather than fixed. See [ADR-0032](../decisions/0032-every-chat-message-pushes.md) |
...
7. **Media cost.** **Half paid off, and the open half was settled by measurement on 2026-08-23 rather than left as an inference.** The scheme built in Phase 3 is the CDN-friendly one this entry asked for, and the mechanism turned out to matter more than expected - see [Media pipeline](../TECH/07-media-pipeline.md) on which of the CDN and the object store signs. Hour-aligned expiry means every viewer inside a window is issued a byte-identical URL, so the cache **key** collapses to one. **What it does not buy is a shared entry.** `cf-cache-status` is absent from a real signed URL served by the deployed CDN, so nothing is held at the edge and N members opening one photo is still N reads of the object store. That confirms [ADR-0044](../decisions/0044-the-cdn-is-a-worker-that-validates-before-it-reads.md), which predicted it and carries what turning the switch on would cost. Originally: Signed URLs are memoized per device, which fixed the repeat-fetch
...
12. ~~**Rate limiting beyond messages**~~ **Done 2026-08-03.** The entry understated it: it was not
    ...
    **Per-conversation and per-club preferences remain open**, as does anything finer than a
    per-user bucket.
13. **Backups and version parity** between development and production data stores. **Still open, and no longer for the reason it gave.** This said "there is no production yet"; there is one as of 2026-08-23, so the gap is a real one rather than a hypothetical: no backup has ever been restored. Migrations already replay cleanly from zero, which is half of it. See [Road to the first club](../TECH/20-road-to-the-first-club.md) milestone 5 for what the first deployment did and did not settle.
...
| **`MEDIA_URL_MODE=cdn`** | **Production, 2026-08-23.** The api serves media in `cdn` mode, the CDN answers on its own hostname, and photos render on the physical iPhone through it. The two sides report the same signing fingerprint, which is what proves one secret rather than two | **A signed URL has now been watched crossing an hour boundary**, so no part of `cdn` mode rests on reasoning: the expiry is hour aligned, so fifty minutes of working proves nothing about the fifty-first. Android, as everywhere |
...
| **Every secret in `.env` is still its development placeholder** | **Closed 2026-08-23**, except four rotations deferred the same day | `BETTER_AUTH_SECRET`, `MEDIA_SIGNING_SECRET` and the S3 credentials were all dev values, to be generated fresh and held by the platform rather than in a file. **They reached all three Fly apps on 2026-08-23**, with `fly secrets import` and never `fly secrets set`, because `set` takes the value as a command argument and so lands it in shell history and the process table. Six on **each** of the three, since every role parses the whole flat schema and refuses to boot on a value it never reads, plus `RESEND_API_KEY`, `MAIL_FROM` and `PLATFORM_MODERATORS` on the api alone. Checked by digest rather than by having typed them carefully: `BETTER_AUTH_SECRET` is one digest across all three, and `MEDIA_SIGNING_SECRET` is deliberately three different ones, because only the api's is real and the other two are throwaways clearing the schema's floor that nothing reads. **The three api-only values are the part reading the schema would have missed**, which is why this row was ever longer than the required set. `RESEND_API_KEY` is `.optional()`, but the mailer assertion throws at boot in production without a transport; `MAIL_FROM` is then forced by a cross-field refine, so a key with no From address fails config loading outright; and `PLATFORM_MODERATORS` fails the opposite way and is the dangerous one, since nothing throws - the api warns that nobody can read the direct-message report queue and boots normally, so the warning is the whole of the enforcement. It was set to a single address on 2026-08-23 and the api logged `configured:1, granted:1, revoked:0, unmatched:[]` on the boot that followed, which is the only evidence that value ever produces. **Four credentials are disclosed, and the founder deferred all four rotations on 2026-08-23** - dated and deliberate, which is neither an oversight nor work that is done. The R2 secret access key, the Cloudflare API token, the older full-access Resend key and a Sentry organization auth token each passed through a chat transcript, and each stays disclosed until it is replaced rather than until enough time passes that it feels unlikely. The R2 credential is read AND write where the CDN only ever reads, so the rotation is also the moment to narrow it. The standing record is [Deployment](../TECH/21-deployment.md), "Obligations that survive the cutover"; what the deployment did and did not settle is [Road to the first club](../TECH/20-road-to-the-first-club.md) milestone 5, and the non-secret values that were missing from the deployment configuration on 2026-08-21 are that milestone's own row rather than this one |
...
| ~~**`MEDIA_URL_MODE=cdn` has never served a byte**~~ | **Closed 2026-08-23** | It serves. The three server apps went out on the object store's own signing first, the CDN was proved on the real hostname while nothing depended on it, and only then did the mode flip - one environment value changed and nothing else, so both states were green independently. Photos render on the physical iPhone through it. The hour-aligned expiry has been watched crossing its boundary and survived it; see [Deployment](../TECH/21-deployment.md) step 6 |
