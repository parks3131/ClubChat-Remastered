...
| **Every secret in `.env` is still its development placeholder** | Blocking for production. **Half closed 2026-08-21** | `BETTER_AUTH_SECRET`, `MEDIA_SIGNING_SECRET` and the S3 credentials are all dev values. They must be generated fresh and held by the platform, never in a file, before anything is deployed. **The providers now exist** - Neon, Upstash via Fly, Cloudflare R2, Sentry and Resend are all provisioned, and the real credentials are collected. What remains is `fly secrets import` against the three apps, which has not run. `import` rather than `set`, because `set` takes the value as a command argument and so lands it in shell history and the process table. The two generated secrets are still to be produced with `openssl rand`, straight into the platform and never into a file, and `MEDIA_SIGNING_SECRET` must differ from `BETTER_AUTH_SECRET` as `.env.example` requires. **Three further values belong on this list that `config.ts` does not mark required, and all three are the api's alone.** `RESEND_API_KEY` is `.optional()` in the schema, but `assertProductionMailer` throws at boot when `NODE_ENV=production` without a transport, and the `Dockerfile` sets `NODE_ENV=production`, so the api role does not start without it. `MAIL_FROM` is then forced by a cross-field refine on the schema, so a key with no From address fails `loadConfig` outright. `PLATFORM_MODERATORS` fails the opposite way and is the dangerous one: nothing throws, the api logs that nobody can read the direct-message report queue and boots normally, so the warning is the whole of the enforcement. Reading the schema alone would miss all three, which is why a list that stops at the required set reads as complete and is not. **Updated 2026-08-21:** four values that are required and are *not* secrets were missing from all three `fly/*.toml` files (`BETTER_AUTH_URL`, `S3_ENDPOINT`, `S3_BUCKET_PUBLIC`, `S3_BUCKET_PRIVATE`), which would have failed the first boot; they are now present and each `[env]` block has been fed through the real `loadConfig` to prove it. `MEDIA_SIGNING_SECRET` is deliberately a **different throwaway on the gateway and the worker**, since neither reads it and both need 16 characters to boot, so `fly secrets list` showing three different digests for one name is correct rather than drift. The api's value must instead be byte-identical to the CDN Worker's, which is what `/__parity` exists to check. **Two credentials are already disclosed and must be rotated as part of the cutover, not after it**: the R2 secret access key and the Cloudflare API token were both pasted into a chat transcript rather than into the secrets file. The R2 credential is read AND write where the Worker only ever reads, so the rotation is also the moment to narrow it. See [Deployment](../TECH/21-deployment.md), "Obligations that survive the cutover" |
