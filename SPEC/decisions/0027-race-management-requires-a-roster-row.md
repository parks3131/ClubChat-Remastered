...
| Follow-up needed | **The client still shows the manage-from-outside screens.** The server refuses correctly, so nothing is exploitable, but a race hub reached by an off-roster admin still offers controls that now answer 404. `PRD/09` rule 5 and `PRD/15`'s race tree are already rewritten; the screens are not. |
